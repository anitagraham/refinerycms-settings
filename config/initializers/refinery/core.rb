...
Refinery::Core.configure do |config|
  ...
  # config.plugin_priority = %w(refinery_pages refinery_images)
    ...
    # Register extra javascript for backend
    # config.register_javascript "refinerycms-settings.js"

    # Register extra stylesheet for backend (optional options)
    config.register_stylesheet "refinerycms-settings.css", media: 'screen'
  end

  # Rails.application.config.assets.precompile += %w( refinerycms-settings.css refinerycms-settings.js )
  Rails.application.config.assets.precompile += %w( refinerycms-settings.css )
